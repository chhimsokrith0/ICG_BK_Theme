...
const Testimonials = () => {
    ...
    return (
        <section className="testimonial-section mx-auto max-w-[1400px] py-8 px-4 bg-gray-50 rounded-lg shadow-lg">
            <p className="text-xl font-semibold mb-6">Testimonials</p>
            <Slider {...settings}>
                {testimonials.map((testimonial, index) => (
                    <div
                        key={index}
                        className="testimonial-box p-6 mx-2 flex flex-col items-center text-center "
                    >
                        <p className="text-lg font-bold mb-4">{testimonial.title}</p>
                        <p className="text-sm text-gray-600 mb-6">{testimonial.description}</p>
