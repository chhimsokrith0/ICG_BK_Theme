...
const InstantGamesToPlay = () => {
  ...
  const GameCard = ({ game, index }: { game: any; index: number }) => (
    ...
    >
      <div className="game-img-wrapper relative group overflow-hidden rounded-xl shadow-lg">
        ...
        />
        {game.label && (
          <div className="absolute top-2 right-2 bg-red-500 text-white px-2 py-0.5 rounded-md text-xs font-medium">
            {game.label}
          </div>
        )}
  ...
  return (
    <motion.section
      className="instant-games-section mx-auto max-w-[1400px] px-4 md:px-8 py-8"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.5 }}
    >
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-gray-900">{t("instantGamesTitle")}</h2>
        <Link href="/Games/fastgames" className="text-sm text-gray-600 hover:text-gray-900">
          {t("seeAll")}
        </Link>
      </div>
