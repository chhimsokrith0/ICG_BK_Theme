...
export default function MobileMenu({ closeMenu }: { closeMenu: () => void }) {
  ...
  const renderMenu = (key: number) => (
    <motion.div
      key={key}
      className="bg-white shadow-md p-4 mt-20 ml-4 sm:ml-10 w-[90vw] sm:w-[653px] max-w-[653px] fixed top-0 left-0 z-50 overflow-y-auto rounded-lg grid grid-cols-1 sm:grid-cols-2 gap-4"
      initial={{ opacity: 0, x: -200 }}
      animate={{ opacity: 1, x: 0 }}
      exit={{ opacity: 0, x: -200 }}
      transition={{ duration: 0.5, ease: "easeInOut" }}
    >
      {/* Left Section */}
      <div className="text-gray-700 text-sm font-medium grid grid-cols-2 sm:grid-cols-1 gap-4 sm:border-l sm:pl-4 sm:justify-items-center">
        {/* Main Section */}
        <div className="space-y-6 mb-6">
          {menuItemsMain.map((item: any, index: number) => (
            <motion.div
            ...
            >
              <Link href={item.link}>
                <div className="flex items-center space-x-2 cursor-pointer">
                  ...
                  <span>{item.label}</span>
                  {item.badge && (
                    <span className={`ml-2 ${item.badge.color} text-xs px-2 py-0.5 rounded-full`}>
                      {item.badge.text}
                    </span>
                  )}
                </div>
