  ...
  return (
    <section id="benefits" className="bg-[#0b2134] py-8 md:py-16 px-4">
      <div className="max-w-[1400px] mx-auto">
        <div className="overflow-x-auto">
          ...
          <div className="bg-gradient-to-r from-[#0d1f2c80] to-[#0b1b27] shadow-md text-gray-300 min-w-[600px]">
            {rows.map((row, index) => (
              ...
              >
                <div className="text-left pl-4 text-xs sm:text-sm md:text-base">{row.benefit}</div>
                {row.values.map((value, i) => (
                  <div
                    key={i}
                    className={`text-xs sm:text-sm md:text-base ${
                      i < row.values.length - 1 ? "border-r border-yellow-600" : ""
                    }`}
                  >
                    {value}
                  </div>
        ...
        {/* Last Updated */}
        <div className="text-gray-500 text-xs sm:text-sm mt-4 text-right">{t("lastUpdated")}</div>
      </div>
    </section>
  )
}
